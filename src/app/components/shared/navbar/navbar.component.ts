...
export class Navbar implements OnInit {
    ...
    constructor(public authService: AuthService, private router: Router, public themeService: ThemeService, private locateService: LocateService) {}
    ...
    ngOnInit() {
        ...
        this.authService.getStatus().subscribe(async (loggedIn: boolean) => {
            this.updateNavbarItems(loggedIn);
        });

        // Subscribe to language changes and update the navbar items
        this.locateService.getCurrentLanguage().subscribe(() => {
            this.updateNavbarItems(this.authService.isLoggedIn());
        });
    }

    private updateNavbarItems(loggedIn: boolean) {
        if (loggedIn) {
            this.currentActor = this.authService.getCurrentActor();
            const role = this.currentActor?.role;
            this.activeRole = role ? role.toString().toLowerCase() : 'anonymous';
        } else {
            this.activeRole = 'anonymous';
            this.currentActor = null;
        }

        // Rebuild the navbar items based on the current language and role
        let commonNavbar = [
            {
                label: this.locateService.translate('Home'),
                icon: 'pi pi-home',
                command: () => this.router.navigate(['/'])
            },
            {
                label: this.locateService.translate('Trips'),
                icon: 'pi pi-map',
        ...
        let configNavbar = [
            ...
            this.themeToggleItem,
            {
                label: this.locateService.translate('ESP'),
                icon: 'pi pi-flag',
                command: () => this.locateService.changeLanguage('es')
            },
            {
                label: this.locateService.translate('ENG'),
                icon: 'pi pi-flag',
                command: () => this.locateService.changeLanguage('en')
            },
        ];
        ...
        if (this.activeRole === 'anonymous') {
            let registerNavbar = [
                {
                    label: this.locateService.translate('Login'),
                    icon: 'pi pi-sign-in',
                    command: () => this.router.navigate(['/login']),
                    style: {'margin-left': 'auto'}
                },
                {
                    label: this.locateService.translate('Register'),
        ...
        let profileNavbar = [
            {
                label: this.locateService.translate('Profile'),
                icon: 'pi pi-user',
                items: [
                    {
                        label: this.locateService.translate('My Profile'),
                        icon: 'pi pi-user-edit',
                        command: () => this.router.navigate([`/user/${this.currentActor?.id}/profile`])
                    },
                    {
                        label: this.locateService.translate('Logout'),
                        icon: 'pi pi-sign-out',
        ...
        switch (this.activeRole) {
            case 'explorer':
                let explorerNavbar = [
                    {
                        label: this.locateService.translate('Explorer'),
                        icon: 'pi pi-compass',
                        style: {'margin-left': 'auto'},
                        items: [
                            {
                                label: this.locateService.translate('My Applications'),
                                icon: 'pi pi-file',
            ...
            case 'manager':
                let managerNavbar = [{
                    label: this.locateService.translate('Manager'),
                    icon: 'pi pi-briefcase',
                    style: {'margin-left': 'auto'},
                    items: [
                        {
                            label: this.locateService.translate('My Trips'),
                            icon: 'pi pi-map',
            ...
            case 'admin':
                let adminNavbar = [{
                    label: this.locateService.translate('Admin'),
                    icon: 'pi pi-cog',
                    style: {'margin-left': 'auto'},
                    items: [
                        {
                            label: this.locateService.translate('Create User'),
                            icon: 'pi pi-user-plus',
                            command: () => this.router.navigate(['/admin/create-user'])
                        },
                        {
                            label: this.locateService.translate('Dashboard'),
                            icon: 'pi pi-chart-line',
                            command: () => this.router.navigate(['/admin/dashboard'])
                        },
                        {
                            label: this.locateService.translate('Sponsorship Configuration'),
                            icon: 'pi pi-cog',
                            command: () => this.router.navigate(['/admin/sponsorship-configuration'])
                        },
                        {
                            label: this.locateService.translate('Cube'),
                            icon: 'pi pi-stop',
            ...
            case 'sponsor':
                let sponsorNavbar = [{
                    label: this.locateService.translate('Sponsor'),
                    icon: 'pi pi-wallet',
                    style: {'margin-left': 'auto'},
                    items: [
                        {
                            label: this.locateService.translate('Create Sponsorship'),
                            icon: 'pi pi-plus',
                            command: () => this.router.navigate(['/sponsorship/create'])
                        },
                        {
                            label: this.locateService.translate('My Sponsorships'),
